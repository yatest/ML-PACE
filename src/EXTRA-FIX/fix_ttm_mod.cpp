  ...
  N_ion(nullptr), N_ion_all(nullptr), switched_on(nullptr)
{
  ...
  // ei_flag == true if temperature dependent g_ei used
  ei_flag = false;
  if (gamma_p == -1) ei_flag = true;
  ...
  if (gamma_p < 0.0 && !ei_flag) error->all(FLERR,"Fix ttm/mod gamma_p must be >= 0.0");
      ...
      if (surf_flag == 0) {
        ...
        if (ix >= surface_l) {
          if (ix < surface_r) {
            ...
            double C_i = el_properties(T_electron[ix][iy][iz],rho_e[ix][iy][iz]).el_heat_capacity;
            double C_ir = el_properties(T_electron[right_x][iy][iz],rho_e[right_x][iy][iz]).el_heat_capacity;
            double C_iu = el_properties(T_electron[ix][right_y][iz],rho_e[ix][right_y][iz]).el_heat_capacity;
            double C_if = el_properties(T_electron[ix][iy][right_z],rho_e[ix][iy][right_z]).el_heat_capacity;
...
double FixTTMMod::lerp(float a, float b, float t)
{
    return a + t * (b - a);
}
...
void FixTTMMod::read_parameters(const std::string &filename)
{
  if (comm->me == 0) {
    try {
      ...
      // gamma_p
      // first value is length of array, second is name of file
      ...
      if (nwords == 2) {
        ...
        file_len = line_token.next_int();
        ei_infile = line_token.next_string().c_str();
        //ei_infile = utils::strdup(arg[1]);
        read_electron_ion(ei_infile,file_len);
      } else {
        auto line_token = ValueTokenizer(line);
        gamma_p = line_token.next_double();
      }
...
/* -----------------------------------------------------------------------
   read in temperature dependent electron-ion coupling
-------------------------------------------------------------------------- */

void FixTTMMod::read_electron_ion(const std::string &filename, int file_len)
{
  if (comm->me == 0) {

    T_el = new double[file_len];
    g_ei = new double[file_len];

    bigint nread = 0;

    try {
      PotentialFileReader reader(lmp, filename, "ttm/mod electron-ion");

      while (nread < file_len) {

        // first column is T_el, second column is g_ei
        auto values = reader.next_values(2);
        ++nread;

        T_el[nread] = values.next_double()*10000.0;
        g_ei[nread] = values.next_double();

        if (T_el < 0)
          throw TokenizerException("Fix ttm electron temperatures in g_ei file must be > 0.0","");
        if (g_ei < 0)
          throw TokenizerException("Fix ttm g_ei in g_ei file must be > 0.0","");
...
el_heat_capacity_thermal_conductivity FixTTMMod::el_properties(double T_e, double rho)
{
  ...
  properties.el_heat_capacity = rho*(poly*exp(-T_reduced*T_reduced) + C_limit); // heat capacity
  properties.el_thermal_conductivity = el_th_diff*properties.el_heat_capacity; // thermal conductivity
  return properties;
}
double FixTTMMod::el_sp_heat_integral(double T_e, double rho)
{
  double T_temp = T_e/1000.0, T_reduced = T_damp*T_temp;
  if (T_damp != 0)
    return rho*(MY_PIS*(3*esheat_4/pow(T_damp,5)+2*esheat_2/pow(T_damp,3)+4*esheat_0/T_damp)*erf(T_reduced)+
                               //4*esheat_3/pow(T_damp,4)+4*esheat_1/T_damp/T_damp+
                               -((6*esheat_4*T_temp+4*esheat_3)/pow(T_damp,4)+
                                (4*esheat_1+4*esheat_4*pow(T_temp,3)+4*esheat_3*T_temp*T_temp+4*esheat_2*T_temp)/T_damp/T_damp)*exp(-T_reduced*T_reduced))*125.0+rho*C_limit*T_e;
  else
    return rho*((esheat_0 + C_limit)*T_e + esheat_1*T_temp*T_e/2.0 + esheat_2*T_temp*T_temp*T_e/3.0 + esheat_3*pow(T_temp,3)*T_e/4.0 + esheat_4*pow(T_temp,4)*T_e/5.0);
}
...
void FixTTMMod::end_of_step()
{
  ...
  if (surf_flag == 0) {
    ...
    double electronic_specific_heat = el_properties(atom->T_e_avg,electronic_density).el_heat_capacity;
    double electronic_thermal_conductivity = el_properties(atom->T_e_avg,electronic_density).el_thermal_conductivity;
    ...
    if (stability_criterion < 0.0) {
      inner_dt = (1.0/6.0)*(electronic_specific_heat) /
        (electronic_thermal_conductivity*(1.0/dx/dx + 1.0/dy/dy + 1.0/dz/dz));
      num_inner_timesteps = static_cast<int>(update->dt/inner_dt) + 1;
    ...
    for (int istep = 0; istep < num_inner_timesteps; istep++) {
      ...
      for (int ix = 0; ix < nxgrid; ix++)
        for (int iy = 0; iy < nygrid; iy++)
          for (int iz = 0; iz < nzgrid; iz++)
            T_electron_old[ix][iy][iz] = T_electron[ix][iy][iz];
        ...
        for (int iy = 0; iy < nygrid; iy++)
          for (int iz = 0; iz < nzgrid; iz++) {
            ...
            T_electron[ix][iy][iz] =
              ...
              inner_dt/(electronic_specific_heat) *
              (electronic_thermal_conductivity *

               ((T_electron_old[iz][iy][xright] + T_electron_old[ix][iy][zleft] -
                 2.0*T_electron_old[ix][iy][iz])/dx/dx +
                (T_electron_old[iz][yright][ix] + T_electron_old[ix][yleft][iz] -
                 2.0*T_electron_old[ix][iy][iz])/dy/dy +
                (T_electron_old[zright][iy][ix] + T_electron_old[xleft][iy][iz] -
                 2.0*T_electron_old[ix][iy][iz])/dz/dz) -
...
{
  double bytes = 0.0;
  // this does not include new arrays added currently
...
double FixTTMMod::compute_vector(int n)
{
  ...
  for (int ix = 0; ix < nxgrid; ix++)
    for (int iy = 0; iy < nygrid; iy++)
      for (int iz = 0; iz < nzgrid; iz++) {
        e_energy += el_sp_heat_integral(T_electron[ix][iy][iz],rho_e[ix][iy][iz])*del_vol;
        transfer_energy +=
...
void FixTTMMod::restart(char *buf)
{
  ...
  // might need to add something here for rho_e
